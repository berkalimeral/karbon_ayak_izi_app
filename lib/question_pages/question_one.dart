...
class _QuestionOneState extends State<QuestionOne> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.question.answerType == 'dropdown') {
      return dropDownQuestion(context, controller);
    } else if (widget.question.answerType == 'radioListTile') {
      return radioListTileQuestion(context);
    } else {
      return textFieldQuestion(context, controller);
    }
  }

  Column textFieldQuestion(
                        ...
                        case 2:
                          {
                            result = elektrikKatsayi *
                                (double.parse(controller.text) / 0.20);
                        ...
                        case 7:
                          {
                            result = ic_ucus_katsayi *
                                (double.parse(controller.text) / 0.9);
                            finalResult.add(result);
                          }
                          break;
                        case 8:
  ...
  Column radioListTileQuestion(BuildContext context) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.spaceEvenly,
      children: [
        QuestionsPage(index: widget.index),
                    ...
                    onTap: () {
                      switch (widget.question.id) {
                        case 1:
                          {
                            if (selectedValue ==
                                widget.question.items![widget.answerIndex].id) {
                        ...
                        case 4:
                          {
                            if (selectedValueArac == 'otobus') {
                              ...
                              result =
                                  tren_katsayi * double.parse(controller.text);
                              debugPrint(result.toString());
                            } else if (selectedValueArac == 'metro') {
                              result =
                                  metro_katsayi * double.parse(controller.text);
                              debugPrint(result.toString());
                            } else {
                              result =
                                  vapur_katsayi * double.parse(controller.text);
                            }
                            finalResult.add(result);
                            debugPrint(finalResult.toString());
                          }
